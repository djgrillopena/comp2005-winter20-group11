...
public class Game {
    ...
    private JButton targetImage;   // the button in the targetPanel that displas the target chip.
    private JLabel targetName;     // contains the name of the image.
    //Non swing variables or logic variables
    private int turn = 5;             //There are 5 chances in the game.
    ...
    public Game() {
        ...
        //initiating the moving hashtable which will hold the number of bid that each player makes in each turn.
        {
    ...
    public void Board() {
        if(third.isSimpleOrComplex())
        {
            ...
            target temptarget = this.TargetChip();
            int x = temptarget.getX();
            int y = temptarget.getY();
            boardButton = board.getBoard();
    ...
    public void leftMove (){
        if(this.isBRYG()) {
            ...
            int x = arr[0];
            int y = arr[1];
            boolean k = boardButton[x][y].getLeftWall();
            System.out.println(k);
            if (!boardButton[x][y].getLeftWall()) {
                boardButton[x][y].removeToken();
                int z = 0;
                y -= 1;
                boolean found = false;
                while (y >= 0) {
    ...
    public void rightMove(){
        if(this.isBRYG()) {
            ...
            int x = arr[0];
            int y = arr[1];
            boolean laskdjf = !boardButton[x][y].getRightWall();
